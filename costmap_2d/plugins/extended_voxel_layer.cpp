...
void ExtendedVoxelLayer::publishClearing()
{
  ...
  for (std::list<std::pair<unsigned int, unsigned int> >::iterator updated_cell_index_it = updated_cells_indices_->begin();
      updated_cell_index_it != updated_cells_indices_->end(); ++updated_cell_index_it)
  {
